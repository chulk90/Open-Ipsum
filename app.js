...
, Generator = require('./GeneratorModel');

mongoose.connect('mongodb://localhost/theopenapi');
...
app.use(express.cookieParser('ipsumipsum'));
app.use(express.session({secret:'iloveopenipsumsomuch', cookie:{ maxAge: 999999}}));
...
// routes
require('./twitterAuthRoutes')(app, passport);
require('./generatorRoutes')(app, Generator);
require('./staticRoutes')(app, Generator);




http.createServer(app).listen(app.get('port'), function(){
  console.log('Its going down on ' + app.get('port'));
});
...
/*
// Generate ipsum route
app.get('/ipsums/:name/generate', function(req, res){
    Generator.find({name: req.params.name}, function(err, docs){
        var lorem = new Lorem;
        Lorem.WORDS = docs[0].words;
        if (req.query.paragraphs){

            res.json(lorem.createText(req.query.paragraphs, Lorem.TYPE.PARAGRAPH));

        } 

        else if(req.query.sentence){
            res.json(lorem.createText(req.query.sentence, Lorem.TYPE.SENTENCE));
        } 

        else if(req.query.word){
            res.json(lorem.createText(1, Lorem.TYPE.WORD));

        }

        else{

            res.json(lorem.createText(1, Lorem.TYPE.PARAGRAPH));
